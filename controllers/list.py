...
class NewPage(http.Controller):
    ...
    @http.route('/get-different-categories', type="http", auth='public')
    def get_categories(self, **kwargs):
        values = dict(kwargs)
        sort_fields = ['', 'name', 'product_category_type'];
        locations_list = []
        totalCount = 0;
        offset = 0
        limit = 10
        ...
        if values['search[value]']:
            rooms = request.env['metrc.categories'].search([('name', 'like', '%'+values['search[value]']+'%')], limit = limit, offset= offset, order= order_text)
            totalCount = request.env['metrc.categories'].search_count([('name', 'like', '%'+values['search[value]']+'%')])
        else:
            rooms = request.env['metrc.categories'].search([], limit = limit, offset= offset, order= order_text)
            totalCount = request.env['metrc.categories'].search_count([])
        for single_room in rooms:
            single_batch_item = ['', single_room.name, single_room.product_category_type]
